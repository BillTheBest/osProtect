...
def create_events(sensor)
  sig = SignatureDetail.where(["sig_name like ?", "%*** notification testing ***%"]).first
  unless sig
    sig = SignatureDetail.create!(sig_class_id: 1, sig_name: "*** notification testing ***", sig_priority: 1)
  end
